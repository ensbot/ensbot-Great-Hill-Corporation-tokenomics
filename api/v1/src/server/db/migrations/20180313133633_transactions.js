exports.up = (knex, Promise) => {
  ...
  // create an address table so that addresses are only stored once
    ...
    table.integer('user_id').unsigned().references('user_id').inTable('user').onDelete('CASCADE'); // delete user monitors if the user is deleted.
    table.integer('monitor_view_id').unsigned().notNullable();
    table.binary('address', 20).references('address').inTable('address').notNullable().onDelete('RESTRICT'); // prevent deletion of an address when it's used in a monitor.
    ...
    table.binary('address', 20).references('address').inTable('address').notNullable().onDelete('RESTRICT'); // Prevent address deletion if we have a transaction for it
    table.integer('block_number').unsigned().notNullable();
    table.integer('tx_index').unsigned().notNullable();
    table.integer('trace_id').unsigned().notNullable();
    table.foreign(['block_number', 'tx_index', 'trace_id']).references(['block_number', 'tx_index', 'trace_id']).inTable('transaction').onDelete('CASCADE'); // Delete the address-transaction mapping if the block or tx gets deleted.
...
exports.down = (knex, Promise) => {
  ...
  // .dropTable('input_data')
    .dropTable('monitor_transaction').dropTable('transaction').dropTable('abi_spec').dropTable('monitor_view').dropTable('address').dropTable('block').dropTable('user').dropTable('price');
};
