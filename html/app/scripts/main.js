...
const xScale = d3.scaleBand().domain(d3.range(dataset.length)).rangeRound([0, w]).paddingInner(0.05);
...
//Create SVG element
const svg = d3.select('#chart').append('svg').attr('width', w).attr('height', h).attr('preserveAspectRatio', 'xMinYMin meet').attr('viewBox', '0 0 ' + w + ' ' + h).classed('svg-content-responsive', true);
...
let timeout = (n) => {
  setTimeout(() => {
    ...
    }).duration(1500).attr('y', function(d) {
      return h - yScale(d.val);
